...
typedef struct {
    ...
    // byte #:  |3  |2  |1  |0
    //
    // Higher byte order give higher priority. Thus, a value of 0x0000000c represents
...
static void setCurrentModemType(int modemType);
                                        ...
                                        size_t datalen, RIL_Token t)
{
    ...
    if (!ss || !datalen) {
        LOGE("RIL_REQUEST_CDMA_SET_SUBSCRIPTION_SOURCE without data!");
        RIL_onRequestComplete(inst_id, t, RIL_E_GENERIC_FAILURE, NULL, 0);
        return;
    }
                                        ...
                                        size_t datalen, RIL_Token t)
{
    ...
    for (cur = p_response->p_intermediates, i = 0; cur; cur = cur->p_next, i++) {
        if (i >= RIL_MAX_NETWORKS) {
            LOGI("Registration %d ignored [max networks: %d]: %s",i,RIL_MAX_NETWORKS,cur->line);
            continue;
        }
...
static void  requestSIM_IO(int inst_id, void *data, size_t datalen, RIL_Token t)
{
    ...
    LOGD("requestSIM_IO(): inst_id = %d", inst_id);
...
onRequest (int inst_id, int request, void *data, size_t datalen, RIL_Token t)
{
    ...
    switch (request) {
        ...
        case RIL_REQUEST_SET_DATA_SUBSCRIPTION_SOURCE:
            ...
            break;
        case RIL_REQUEST_GET_UICC_SUBSCRIPTION_SOURCE:
            getUiccSubscriptionSource(inst_id, request, data, datalen, t);
            break;
        case RIL_REQUEST_GET_DATA_SUBSCRIPTION_SOURCE:
...
static void pollSIMState (void *param)
{
    ...
    /*TODO: find out if we still need to poll and return if not...
...
static void onUnsolicited (const char *s, const char *sms_pdu)
{
    ...
    if (strStartsWith(s, "%CTZV:")) {
        ...
        RIL_onUnsolicitedResponse (
            ...
            NULL, 0);
#if 0 // WORKAROUND_FAKE_CGEV // TODO: Need this here?
